 ...
 *    b). 通过UNSAFE操作(包括CAS、get、set等)加while循环(自旋)去创建Segment数组中的每个Segment,
 *        保证了多线程情况下的线程安全;
 *    c). 每个Segment使用各自独立的ReentrantLock锁(Segment内部类本身就是继承自ReentrantLock),
 *        每个Segment之间互不影响, 所以提高了并发效率(分段锁的思想);
 ...
 *    另外它保证线程安全的方式是利用UNSAFE操作(包括CAS、get、set等)和synchronized; 具体来说,
